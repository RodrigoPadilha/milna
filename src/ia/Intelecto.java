...
public class Intelecto {
    ...
    private static Intelecto UNIQUE_INSTANCE;
    private Grafo grafo;
    ...
    public static synchronized Intelecto getInstance() {
        if (UNIQUE_INSTANCE == null)
            UNIQUE_INSTANCE = new Intelecto();
    ...
    public void inicio(Mapa mapa) {
        ...
        if (grafo.getMapa().getEstadoIni().getNome() == null) {        // Se NãO foi selecionado um ponto inicial tenta se localizar
            ...
            comportamentoInteligente(estadoIni, new int[]{0, 0}); // Passa os valores para a primeira leitura do rob�
			/*
...
//		ArrayList<Estado> possibilidades = localizador.buscaLocalizacao();
        ...
        if (perdido) {                // Sim
            ...
            if (possibilidades.size() > 1) {
                ...
                System.out.println("Localiza��o conclu�da");
